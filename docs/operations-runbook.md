...
7. Результат записывается в поля Kommo `qlead_<platform>_status`,
   `qlead_<platform>_detail`, `qlead_<platform>_sent_at`.
...
3. Проверить четыре набора полей `qlead_*` в Kommo.
